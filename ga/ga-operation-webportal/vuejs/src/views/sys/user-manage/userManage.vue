    ...
    <div style="width: 1280px; height: 900px; margin-top:30px; background:white">
        ...
        <Modal :title="modalTitle" v-model="userModalVisible" :mask-closable='false' :width="500" :styles="{top: '30px'}">
            <Form ref="userForm" :model="userForm" :label-width="70" :rules="userFormValidate">
                ...
                <FormItem label="员工类型" prop="">
                    <!--<Input v-model="userForm.jobTitle"/>-->
                    <Select v-model="userForm.jobTitle"  placeholder="请选择用户类型">
                ...
                <Form-item label="所属部门" prop="departmentTitle">
                  <Poptip trigger="click" placement="right" title="选择部门" width="250">
                    ...
                    <div slot="content">
                      <Tree v-show="tres" :data="dataDep" :load-data="loadDataTree" @on-select-change="selectTree"></Tree>
                        <Tree v-show="tress" :data="dataDep"  @on-select-change="selectTree"></Tree>
                ...
                <FormItem label="所属企业" style="display: none">
                        <Input v-model="userForm.companyId" />
                </FormItem>
...
export default {
  ...
  methods: {
    ...
    getUserList() {
      ...
      this.loading = true;
        let userss = JSON.parse(Cookies.get("userInfo"));
        let cid = userss.companyId;
      getUserListData(cid,this.searchForm).then(res => {
        this.loading = false;
        if (res.success === true) {
      ...
      this.$refs.userForm.validate(valid => {
        if (valid) {
          if (this.modalType === 0) {
            ...
            addUser(type,this.userForm).then(res => {

    ...
    <div class="search">
        <Card>
          <Row class="operation" style="margin-bottom: 10px;">
            <Button v-show="adds" @click="add" type="primary" icon="md-add">添加子部门</Button>
            <Button v-show="addss" @click="addRoot" icon="md-add">添加企业</Button>
          ...
          <Row type="flex" justify="start" class="code-row-bg">
            <Col span="6">
              ...
              <Tree v-show="tres" :data="data" :load-data="loadData" show-checkbox @on-check-change="changeSelect" @on-select-change="selectTree"></Tree>
              <Tree v-show="tress" :data="data"  @on-check-change="changeSelect" show-checkbox @on-select-change="selectTree"></Tree>
              <Spin size="large" fix v-if="loading"></Spin>
            </Col>
            <Col span="9" >
              <Form  ref="form" :model="form" :label-width="85" :rules="menuFormValidate">
                <FormItem v-show="flg" label="上级" prop="parentTitle">
                  <Poptip trigger="" placement="right-start" title="选择上级" width="250">
                ...
                <FormItem v-show="flg" label="部门名称" prop="title">
                  <Input v-model="form.title"/>
                </FormItem>
                <FormItem v-show="flgs" label="企业名称" prop="title">
                  <Input v-model="form.title"/>
                </FormItem>
        ...
        <Modal :title="modalTitle" v-model="menuModalVisible" :mask-closable='false' :width="500">
          <Form ref="formAdd" :model="formAdd" :label-width="85" :rules="menuFormValidate">
            ...
            <FormItem v-show="qy" label="企业名称" prop="title">
              <Input v-model="formAdd.title"/>
            </FormItem>
            <FormItem v-show="qys" label="部门名称" prop="title">
              <Input v-model="formAdd.title"/>
            </FormItem>
            <FormItem label="排序值" prop="sortOrder">
  ...
  methods: {
    ...
    getParentList() {
      this.loading = true;
        ...
        let type = userss.type
        let departmentId = userss.departmentId
      initDepartment({type:type,departmentId:departmentId}).then(res => {
        this.loading = false;
        if (res.success === true) {
    ...
    loadData(item, callback) {
        ...
        let departmentId = userss.departmentId
      loadDepartment(item.id,{type:1,departmentId:departmentId}).then(res => {
        if (res.success === true) {
          res.result.forEach(function(e) {
            if (e.isParent) {
              e.loading = false;
    ...
    submitEdit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if(this.flg){
              if(this.form.parentTitle=="一级部门"){
              this.$Message.warning("无权修改一级部门");
    ...
    delAll() {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          deleteDepartment(ids,{idsp:idsp}).then(res => {
            if (res.success === true) {

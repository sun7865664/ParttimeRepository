    ...
    <div>
        ...
        <!--<Form :label-width="120" style="margin:-15px 10px;">-->
            ...
            <!--<span style="width:100%;display:inline-block;text-align:right;">-->
                ...
                <!--<Button type="primary" @click="addmodel('formvali')" icon="plus"-->
                        <!--style="width:150px;margin-right:10px;" size="small">增  加</Button>-->
            <!--</span>-->
        ...
        <Modal v-model="add_dialog_show" width="700"  title="新增培训证书记录" @on-ok="submitCheckForm"
            ...
            <Form :label-width="100" :model="trainingCertificate" style="height: 100%" ref="formvali" :rules="rule">
                <FormItem label="证书类型" style="width: 50%;margin-top: 12px;height: 34px;margin-bottom: 24px" class="bottom5" prop="certificatetype">
                    <!--<Select v-model="trainingCertificate.certificatetype" placeholder="请选择证书类型" >-->
                        <!--<Option :value="item.key" v-for="item in items">{{item.value}}</Option>-->
                    <!--</Select>-->
...
//                items:employeedata.certificatetype,
        ...
        methods: {
                ...
                self.$refs.formvali.validate((valid) => {
                    if(valid) {
                        ...
                        if ((tc.issuetime.valueOf()) > (tc.expiretime.valueOf())) {
                            ...
                            self.$http.jsonPost('/eim/api/trainingcertificate/savaTrainingCertificate', self.trainingCertificate).then(function (res) {
                                let result = res.data;
            ...
            removeCheck(index) {
                let self = this;
                let url = "/eim/api/trainingcertificate";

    ...
    <div style="width: 1280px; height: auto; background:white">
        <div style="margin-left: 9px;">
            ...
            <div class="bottom5" style="width: 1160px; margin-left: 10px">
                <Table border :columns="columns7" :data="formValidate.pcRecords"></Table>
            </div>
        </div>

        <Modal v-model="modal1" title="熟练检查记录" width="700px" @on-ok="submit" @on-cancel="dealcancel" :styles="{top: '20px'}">
            <Form ref="formvali" :rules="rule" :model="pcRecord" :label-width="150" style="height: 100%;">
                <FormItem style="display: none">
                    <Input v-model="pcRecord.id"></Input>
                </FormItem>

                <div>
                <FormItem class="bottom5" label="定期检查记录标单号" prop="labelNo"
                          style="width: 45%;height: 34px;margin-bottom: 24px;display: inline-block">
                    <Input v-model="pcRecord.labelNo" placeholder="定期检查记录标单号"></Input>
                </FormItem>
                <FormItem class="bottom5" label="检查单位名称" prop="checkUnit"
                          style="width: 45%;height: 34px;margin-bottom: 24px;display: inline-block">
                    <Input v-model="pcRecord.checkUnit" placeholder="检查单位名称"></Input>
                </FormItem>
                </div>

                <div>
                <FormItem class="bottom5" style="width: 45%;height: 34px;margin-bottom: 24px;display: inline-block" label="定期检查设备类型"  prop="type">
                    <Input v-model="pcRecord.type" placeholder="定期检查设备类型"></Input>
                </FormItem>
                <FormItem class="bottom5" style="width: 45%;height: 34px;margin-bottom: 24px;display: inline-block" label="设备型号" prop="equipmentModel">
                    <Input v-model="pcRecord.equipmentModel" placeholder="设备型号"></Input>
                </FormItem>
                </div>
                ...
                <FormItem class="bottom5" style="width: 45%;height: 34px;margin-bottom: 24px;display: inline-block" label="检查日期" prop="checkDate">
                    <DatePicker type="date" placeholder="检查日期" placement="bottom"
                                v-model="pcRecord.checkDate"></DatePicker>
                </FormItem>
                <FormItem class="bottom5" style="width: 45%;height: 34px;margin-bottom: 24px;display: inline-block" label="检查时间" prop="checkTime">
                    <DatePicker type="date" placeholder="检查时间" placement="bottom"
                                v-model="pcRecord.checkTime"></DatePicker>
                </FormItem>
                </div>
                ...
                <FormItem class="bottom5" style="width: 45%;height: 34px;margin-bottom: 24px;display: inline-block" label="检查人"
                          prop="checkPerson">
                    <Input v-model="pcRecord.checkPerson" placeholder="检查人"></Input>
                </FormItem>

                <FormItem class="bottom5" style="width: 45%;height: 34px;margin-bottom: 24px;display: inline-block" label="检查结果"
                          prop="checkResult">
                    <Input v-model="pcRecord.checkResult" placeholder="检查结果"></Input>
                </FormItem>
                </div>
                ...
                <FormItem class="bottom5" style="width: 45%;height: 34px;margin-bottom: 24px;display:inline-block" label="下次检查期限" prop="nextTerm">
                    <DatePicker type="date" placeholder="下次检查期限" placement="bottom"
                                v-model="pcRecord.nextTerm"></DatePicker>
                </FormItem>

                <FormItem class="bottom5" style="width: 45%;height: 34px;margin-bottom: 24px;display: inline-block" label="定期检查工作实践单"
                          prop="checkList">
                    <Input v-model="pcRecord.checkList" placeholder="定期检查工作实践单"></Input>
                </FormItem>
                </div>
...
<script>
    ...
    export default {
        data() {
            return {
                rule: {
                    labelNo: [{required: true, message: '定期检查记录标单号不能为空', trigger: 'blur'}],
                    ...
                    checkList: [{required: true, message: '定期检查工作实践单不能为空', trigger: 'blur'}],
                },
                visible: false,
                modal1: false,
                uploadListarray: [],
        ...
        methods: {
            ...
            init: function (id) {
                let self = this;
                self.formValidate.pcRecords = [{}];
                self.$http.httpGet('/eim/api/periodicalCheckRecord/serach/getbypilotId', {
                    pilotId: id,
                }).then(function (response) {
                    console.log(response, '.................');
                    let datainfo = response.data;
                    self.formValidate.pcRecords.splice(0, 1);
                    console.log(datainfo);
                    for (var i = 0; i < datainfo.length; i++) {
                        self.formValidate.pcRecords.push(datainfo[i]);
                    }
                })
            },
            //添加框
            ...
            updateCheck(index) {
                //alert('xxxx');
                this.pcRecord = this.formValidate.pcRecords[index];
...
//                this.update_dialog_show(id);
            ...
            removeCheck(index) {
                let self = this;
                let id = self.formValidate.pcRecords[index].id;
                self.$http.httpDelete('/eim/api/periodicalCheckRecord/' + id, {}
                ).then(function (response) {
                    alert("删除成功");
                    self.init(self.pilotId);
                }).catch(function (error) {
            ...
            submit: function () {
                ...
                this.$refs.formvali.validate((valid) => {
                    ...
                    if (valid) {
                        ...
                        self.pcRecord.pilotId = self.pilotId;
                        self.$http.jsonPost('/eim/api/periodicalCheckRecord' , self.pcRecord).then(function (response) {
                            self.pcRecord = {};
                            self.init(self.pilotId);
                        }).catch(function (error) {
                            console.log(error);

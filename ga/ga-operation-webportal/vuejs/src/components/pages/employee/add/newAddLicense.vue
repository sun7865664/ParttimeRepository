<template>
    <div style="width: 1280px;display:inline-table;margin-left:15px;margin-top:30px; background:white">
        ...
        <Form :label-width="205"  v-show=controllicenseshow >
            <controllicense ref="control"></controllicense>
        </Form>
        ...
        <div  style="margin-top:10px;" v-show="checkrecordshow">
                <span style="width:100%;display:inline-block;text-align:right;">
                   ...
                   <!-- <Button type="primary" @click="add_dialog_show=true" icon="plus"
                    ...
                    <Button type="primary" @click="add_dialog_show=true" icon="plus" style="width: 98px;height: 32px;margin:24px 60px 11px 0px;border: 1px dashed #2b82da;background: #DDF4FF;font-size: 14px;color: #2FA5FB;" size="small">增  加</Button>
                </span>

            <div style="margin-top:5px;width: 1160px;margin-left: 60px">
    ...
    <!--<div style="width: 1280px; height: 900px; margin-top: 60px;background:white">
        <Form :label-width="140" :model="qualification" ref="formVali" style="padding-top: 40px;padding-left: 40px">
            <div>
                <FormItem label="请选择执照类别" prop="licensetype" style="display: inline-block;width: 500px" :rules="{required: true,message: '请选择执照类别', trigger: 'change'}">
                    <Select v-model="qualification.licensetype" placeholder="请选择" @on-change="changeExt">
                        <Option value="气象执照">气象执照</Option>
                        <Option value="情报执照">情报执照</Option>
                        <Option value="管制执照">管制执照</Option>
                ...
                <FormItem label="执照编号" prop="licenseno" style="display: inline-block;width: 500px"
                          :rules="{required: true, message: '执照编号不能为空', trigger: 'blur'}">
                    <Input v-model="qualification.licenseno" placeholder="请输入"></Input>
                </FormItem>
            </div>
            <div>
                <FormItem label="是否长久有效" style="display: inline-block;width: 500px">
                    <RadioGroup v-model="radio" style="color: #016bd9">
                        <Radio label="yes">是</Radio>
                        <Radio label="no">否</Radio>
                    </RadioGroup>
                </FormItem>
                <FormItem label="执照颁发时间"  prop="issuedate" style="width: 500px;display: inline-block" :rules="{required: true, type: 'date',message: '请选择日期', trigger: 'change'}">
                    <DatePicker v-model="qualification.issuedate" type="date" placeholder="选择颁发时间" style="width: 361px"></DatePicker>
                </FormItem>
            </div>

            <div v-if="radio=='no'">
                <FormItem label="执照到期时间" style="width: 500px">
            ...
            <div style="">
                <div >
                    <uploadfile ref="upload" :uploadList=uploadList v-on:listenTochildEvent="getuploadList"></uploadfile>
                    <h4 style="font-size: 0.7rem;text-align: left; color: lightslategray;margin-left: 10px">
                      执照扫描件<br>图片小于2M尺寸小于640*480</h4>
                </div>
            </div>
        </Form>
        ...
        <Form :label-width="120" v-show=flightdislicenseshow>
            <flightdislicense ref="flightdis" :flightdispatcher=flightDispatcherLicense
                              v-on:getChildMsg="getLicenseExt"></flightdislicense>
        ...
        <Form :label-width="120" v-show=controllicenseshow>
            <controllicense ref="control"></controllicense>
        </Form>
        ...
        &lt;!&ndash;<div class="bottom5"
             style="height:190px;border:1px solid rgba(1,107,217,.5);background-color:rgba(193,220,248,.5);">
            <h2>执照扫描件</h2>
            <uploadfile ref="upload" :uploadList=uploadList v-on:listenTochildEvent="getuploadList"></uploadfile>
            <h4 style="font-size: 0.7rem;text-align: left; color: lightslategray;margin-left: 10px">
                图片小于2M尺寸小于640*480</h4>
        </div>&ndash;&gt;
            ...
            <Form :label-width="100" v-model="qualification">
                <div>
                    <Row>
                        <Col :md="23">
                            <FormItem label="签注时间" class="bottom5">
                                <DatePicker v-model="check_form.licenseCheckDate" type="date"
                                            placeholder="选择日期"></DatePicker>
                            </FormItem>
                        </Col>
                        <Col :md="23">
                            <FormItem label="签注内容" class="bottom5">
                                <Input v-model="check_form.licenseCheckContent" type="textarea" :rows="4"
                                       placeholder="请输入签注内容..."></Input>
                            </FormItem>
                        </Col>
                    </Row>
                </div>

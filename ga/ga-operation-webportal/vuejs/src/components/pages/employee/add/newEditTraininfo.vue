    ...
    <div style="width: 1280px; height: auto; margin-top: 20px;background:white">
        <div style="margin-top: 16px;margin-left: 9px;">
                <!--<span style="width:100%;display:inline-block;text-align:right;">
                    <button @click="add_dialog_show(('formvali'))" data-v-6cb0b875="" type="button"
                            ...
                            style="width: 133px; height: 32px; margin: 24px 56px 11px 0px; border: 1px dashed rgb(43, 130, 218); background: rgb(178, 214, 251); font-size: 14px; color: rgb(36, 128, 221);">&lt;!&ndash;&ndash;&gt; <i
        ...
        <Modal v-model="modal1" title="培训信息" width="700px" @on-ok="submittraining" @on-cancel="dealcancel" :styles="{top: '20px'}">
            ...
            <Form ref="formvali" :rules="rule" :model="trainingRecord" :label-width="100" style="height: 100%;">
                ...
                <FormItem class="bottom5" label="培训类型" prop="trainType"
                          style="width: 50%;height: 34px;margin-bottom: 24px">
                    <Input v-model="trainingRecord.trainType" placeholder="培训类型"></Input>
                </FormItem>

                <FormItem class="bottom5" label="培训科目" prop="trainSubject"
                          style="width: 50%;height: 34px;margin-bottom: 24px">
                    <Input v-model="trainingRecord.trainSubject" placeholder="培训科目"></Input>
                </FormItem>

                <FormItem class="bottom5" style="width: 50%;height: 34px;margin-bottom: 24px" label="培训内容"
                          prop="trainContent">
                    <Input v-model="trainingRecord.trainContent" placeholder="培训内容"></Input>
                </FormItem>

                <FormItem class="bottom5" label="开始时间" style="width: 50%;height: 34px;margin-bottom: 24px"
                          prop="beginDate">
                    <DatePicker type="date" placeholder="开始时间" placement="bottom"
                                v-model="trainingRecord.beginDate"></DatePicker>
                </FormItem>

                <FormItem class="bottom5" label="结束时间" style="width: 50%;height: 34px;margin-bottom: 24px"
                          prop="endDate">
                    <DatePicker type="date" placeholder="结束时间" placement="bottom"
                                v-model="trainingRecord.endDate"></DatePicker>
                </FormItem>

                <FormItem class="bottom5" style="width: 50%;height: 34px;margin-bottom: 24px" label="考核结果"
                          prop="result">
...
//                this.update_dialog_show(id);
            ...
            removeCheck(index) {
                ...
                self.$http.httpDelete('/app/api/training/deleteTrainingRecordById/' + id, {}
                ).then(function (response) {
                    alert("删除成功");
                    self.init(self.employeeId);
                }).catch(function (error) {
            ...
            submittraining: function () {
                this.$refs.formvali.validate((valid) => {
                    ...
                    if (valid) {
                        ...
                        self.$http.jsonPost('/app/api/training', self.trainingRecord).then(function (response) {

...
<template >
    <div style="width: 965px; margin-left: 12px;">
        <div style="background: #2e82dc;height: 36px;padding-top: 6px;margin: 22px 0px 3px 0px;padding: 6px 0 0 0;">
            ...
            <span style="font-size: 16px;color: rgba(255, 255, 255, 1);">事务处理-请求事项</span>
        </div>
            ...
            <div class="physicalp" >
                <div  style="margin-top:10px;">
                    <div class="bottom5" style="margin-top:5px;padding:10px;">
                        <Table border :columns="columns7" :data="reqlist"></Table>
                    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            init: function (obj) {
                let self = this;
                    self.$http.httpGet('/app/api/processrun/list',{
                        pageSize:self.pagesize,
                        currentPage:obj,
                        processName:self.formItem.processName,
                       //begincreatetime:self.formItem.begincreatetime
                        }
                    ).then(function (response) {
                        let datainfo = response.data.data;
                        self.reqlist=datainfo.list;
